...
class JevPolicy:
    ...
    def _world12_opening_override(self, state, action):
        ...
        The generic controller released A while descending into this pair on
        the first World 1-2 attempt, then tried to brake left after landing
        too close to the enemies.  Keep this profile deliberately narrow:
        only the early pair in World 1-2 can override Jev's action, and the
        low-ceiling geometry changes the timing instead of disabling the
        override.
        """
        if self.level_profile != "world1-2-opening":
            return action, None
        ...
        goombas = sorted(
            (
                obj
                for obj in state.get("nearby_objects", [])
                if obj.get("kind") == "goomba"
                and not obj.get("defeated", False)
                and 0 <= obj.get("dx", -1) <= 128
            ),
            key=lambda obj: obj["dx"],
        )
        if (
            mario["x"] <= 280
            and len(goombas) >= 2
            and goombas[1]["dx"] <= 128
        ):
            nearest = goombas[0]["dx"]
            if mario["grounded"]:
                # This pair is beneath a low ceiling. Suppress a generic
                # early jump (the first attempt launched at dx=80 and landed
                # too soon) and keep moving until a short hop is properly
                # spaced.
                if nearest > 48:
                    return "right_run", "world12_pair_approach_under_ceiling"
                if state.get("jump_already_held", False):
                    return "right_run", "world12_pair_rearm"
                return "right_run_jump", "world12_pair_short_hop"
            if corridor.get("low_ceiling_now"):
                return "right_run", "world12_pair_release_under_ceiling"
            if nearest <= 112:
                return "right_run_jump", "world12_pair_sustain_jump"

        # A later green Koopa caused the next observed failure. Keep this
        # second profile tied to its measured World 1-2 section so World 1-1
        # and the rest of the generic controller remain unchanged.
        if not 760 <= mario["x"] <= 960:
            return action, None
        koopas = sorted(
            (
                obj
                for obj in state.get("nearby_objects", [])
                if obj.get("kind") == "green_koopa"
                and not obj.get("defeated", False)
                and obj.get("same_height", True)
                and 0 <= obj.get("dx", -1) <= 80
            ),
            key=lambda obj: obj["dx"],
        )
        if not koopas:
        ...
        if mario["grounded"]:
            if 48 < nearest <= 64:
                return "right_run", "world12_koopa_approach"
            if nearest <= 48:
                if state.get("jump_already_held", False):
                    return "right_run", "world12_koopa_rearm"
                return "right_run_jump", "world12_koopa_short_hop"
        elif nearest <= 64:
            if corridor.get("low_ceiling_now"):
                return "right_run", "world12_koopa_release_under_ceiling"
            return "right_run_jump", "world12_koopa_sustain_jump"
        return action, None
    ...
    def choose(self, state):
        ...
        action, level_override = self._world12_opening_override(state, action)
        diagnostics = {
        ...
        if self.level_profile:
            diagnostics["level_profile"] = self.level_profile
        if level_override:
            diagnostics["level_override"] = level_override
